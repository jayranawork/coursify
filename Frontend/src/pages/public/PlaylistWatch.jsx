...
import { ArrowLeft, Clock3, Keyboard, PanelRightClose, PanelRightOpen, PlayCircle, SkipForward, Video } from "lucide-react";
...
export function PlaylistWatch() {
  ...
  useEffect(() => {
    const resumeVideo = videos.find((video) => String(video._id) === String(playlist.resumeVideoId) && video.isAvailable !== false);
  ...
  const previousVideo = useMemo(
    () => videos.slice(0, Math.max(0, activeVideoIndex)).reverse().find((video) => !isVideoUnavailable(video)) || null,
    [videos, activeVideoIndex, isVideoUnavailable]
  );
  ...
  if (watchQuery.isLoading) return <div className="focus-room-page min-h-screen"><LoadingSpinner label="Opening Focus Room..." className="text-[var(--focus-text-secondary)]" /></div>;
  if (watchQuery.isError) {
    return <div className="focus-room-page min-h-screen p-6"><ErrorState className="focus-room-elevated border" description="We could not load this playlist." onRetry={() => watchQuery.refetch()} /></div>;
  }

  if (!playlist._id) {
    return <div className="focus-room-page min-h-screen p-6"><ErrorState className="focus-room-elevated border" title="Playlist unavailable" description="This playlist could not be found." /></div>;
  }
    ...
    <div className="focus-room-page min-h-screen">
      <div className="focus-room-glass sticky top-0 z-30 border-b px-4 py-4">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <button type="button" onClick={goBack} className="focus-room-control inline-flex items-center gap-2 text-sm font-medium text-[var(--focus-text-secondary)] hover:text-white">
            <ArrowLeft className="h-4 w-4" />
            Back
          </button>
          <div className="min-w-0 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--focus-text-muted)]">Focus Room</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant={playlist.status === "completed" ? "success" : "secondary"} className={playlist.status === "completed" ? "" : "border border-[var(--focus-accent)] bg-[var(--focus-accent-subtle)] text-[var(--focus-accent)]"}>
      ...
      <div className={`mx-auto grid max-w-7xl gap-6 px-4 py-6 ${showQueue ? "lg:grid-cols-[minmax(0,1fr)_380px]" : "lg:grid-cols-1"}`}>
        <div className="space-y-6">
        <Card className="focus-room-surface overflow-hidden rounded-[var(--focus-radius-lg)] border text-white">
          <div className="p-4 sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="truncate text-sm text-[var(--focus-text-secondary)]">{activeVideo?.title || "Select a video"}</p>
          ...
          {showShortcuts ? (
            <div className="focus-room-elevated mx-4 mb-4 rounded-[var(--focus-radius-md)] border p-4 text-sm text-[var(--focus-text-secondary)]" role="dialog" aria-modal="true" aria-labelledby="shortcut-title">
              <div className="flex items-center justify-between gap-4">
                <p id="shortcut-title" className="font-semibold text-white">Keyboard shortcuts</p>
                <button ref={shortcutCloseRef} type="button" className="focus-room-control text-[var(--focus-text-secondary)] hover:text-white" onClick={() => setShowShortcuts(false)} aria-label="Close keyboard shortcuts">Close</button>
              </div>
          ...
          <div className="px-4 pb-4 sm:px-6 sm:pb-6">
            {activeVideo && !activeVideoUnavailable ? (
              <YouTubePlayer
                ...
                onEnded={handleEnded}
                onError={(error) => {
                  if ([100, 101, 150].includes(Number(error?.code))) {
            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.2em] text-[var(--focus-text-muted)]">Watching now · Video {currentVideoPosition} of {videos.length}</p>
                <div className="mt-2 flex items-center gap-3">
                  <Progress value={currentPlaybackPercent} className="w-56 bg-[var(--focus-border)]" indicatorClassName="focus-room-progress" />
                ...
                <p className="mt-1 text-xs text-[var(--focus-text-muted)]">Playlist completion: {playlist.progressPercent || 0}%</p>
              </div>
              <Button
                variant="outline"
                className="focus-room-secondary"
        ...
        {showQueue ? <Card className="focus-room-elevated h-fit rounded-[var(--focus-radius-lg)] border p-4 text-white lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-hidden">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--focus-text-muted)]">Playlist videos</p>
              <h2 className="mt-1 text-xl font-bold">{videos.length} items</h2>
            </div>
            <Badge variant="outline" className="border-[var(--focus-border-strong)] bg-transparent text-[var(--focus-text-secondary)]">{playlist.status}</Badge>
          </div>
          <p className="mb-4 text-xs text-[var(--focus-text-muted)]">Video {currentVideoPosition} of {videos.length}</p>
...
function Shortcut({ keys, label }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span>{label}</span>
      <kbd className="rounded border border-[var(--focus-border-strong)] bg-black/20 px-2 py-1 text-xs text-white">{keys}</kbd>
    </div>
  );
}

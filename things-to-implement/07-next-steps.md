# Next Steps

If you want the cleanest path forward, do these in order.

## Step 1

- Coupon calculation and redemption tracking in the backend are complete.

## Step 2

- Rate limiting and security headers are complete.

## Step 3

- Refresh tokens are persistent.

## Step 4

- Finish the upload pipeline for lesson PDFs and videos with S3 access rules.

## Step 5

- Add video processing and streaming support.

## Step 6

- Reconcile frontend and backend behavior for checkout and auto-enrollment.

## Step 7

- Finish the admin CRUD gaps and polish the UI.

## Step 8

- Split large frontend work if the bundle remains too heavy.
